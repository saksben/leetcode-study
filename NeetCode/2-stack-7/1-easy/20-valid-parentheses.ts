...
function isValid(s: string): boolean {
  ...
  const stack: string[] = []; // Create the stack array that will contain opening parenths
  ...
  for (const char of s) {
    if (char in pairs) {
      const top = stack.pop(); // If all closing parenths have been run through, return a # to trigger in case there is another opening parenth. Otherwise, pop the latest parenth out and make it the topElement

      // If the char's corresponding closing parenth doesn't match the stack's top element, return false early
      if (top !== pairs[char]) {
        return false;
      }
    } else {
